...
def main():
    ...
    for i in range(1, int(sys.argv[4])):
        ...
        pkt = pkt / IP(dst=addr, proto=17) / UDP(dport=4321, sport=1234) / MRI(count=0, swtraces=[]) / str(RandString(size=252))
        pkt.show2()
        print(len(pkt))
        if count_send == 0:
            send_time = pkt.time
        sendp(pkt, iface=iface, inter=0, verbose=False, count = 500)
        count_send = count_send + 1
    print(count_send)
    print(send_time)    
    end_time = time.time()
    print("Start Sending Time:", start_time)
    print("End Sending Time:", end_time)
